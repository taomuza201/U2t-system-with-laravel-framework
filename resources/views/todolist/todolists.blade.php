...
for($i=1;$i<=3000;$i+=4){ $marks_start[]=$i; } for($i=0;$i<=3000;$i+=4){ $marks_end[]=$i; } @endphp @foreach($todolist
    ...
    <div class="col">
        <div class="card-deck">
            ...
            <div class="card rounded shadow-lg" onclick="showdataTodolist(this)" data-id="{{$row->todolists_id}}">
                <div class="card-body">
                    <h5 class="card-title">{{$row->todolists_title}}</h5>
                    <p class="card-text">   {{ \Illuminate\Support\Str::limit($row->todolists_detail, 50, $end='...') }}.</p>
